...
export type WordProps = {
  ...
  types: {
    type: string
    meanings: string // Đổi từ string[] sang string
    notes: string[]
  }[]
}

export function WordForm({ word }: { word: WordProps }) {
  ...
  const [newWordForm, setNewWordForm] = useState({
    ...
    types: word.types || [{ type: "", meanings: "", notes: [""] }],
  })
  ...
  return (
    <form onSubmit={handleUpdateWord} className="mt-4">
      ...
      <div className="space-y-6">
          ...
          {newWordForm.types.map((t, typeIndex) => (
            ...
            >
              <div className="flex justify-between items-center">
                <h4 className="font-medium text-lg">Loại từ {typeIndex + 1}</h4>
                {typeIndex > 0 && (
                  <Button
                    type="button"
                    variant="ghost"
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      handleWordFormChange(
                        { target: { value, name: "type" } } as React.ChangeEvent<HTMLInputElement>,
                        typeIndex,
                        "type"
                      )
                    }
        ...
        <Button type="button" variant="outline" onClick={addType} className="gap-2 w-full">
